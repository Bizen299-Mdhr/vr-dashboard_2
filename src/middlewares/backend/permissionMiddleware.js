...
const {admin, role, userRole} = require("@models");
const PROTECTED_ATTRIBUTES = ['password'];
const {getModulePermissions, getUserPermissions, hasSuperAdminRole} = require('@lib');
module.exports = {
        ...
        return function (req, res, next) {
            ...
            if (req.session.user) {
                if (hasSuperAdminRole(req.session.user)) {
                    ...
                    if (req.session.userPermissions && req.session.userPermissions !== null && _.includes(req.session.userPermissions, permission)) {
                        return next();
                    }
    ...
    sessionUserPermissions: async function (req, res, next) {
        ...
        if (req.session.user && req.session.user.id) {
            if (req.session.user && req.session.user.role_id && req.session.user.role_id !== "") {
