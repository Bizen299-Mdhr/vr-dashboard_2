...
const {permission, role, admin, config} = require('@models');
...
module.exports = {
    up: async () => {
        ...
        let hashRound = 10;
        let hashRoundData = await config.findOne({where:{'name': "Password Hashing Rounds"}}); //get Hashing rounds
        ...
        const superAdmindata = await admin.findOne({where:{
            _id:MAINSUPERADMIN
        }});
