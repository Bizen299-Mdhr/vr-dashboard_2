...
const {hasSuperAdminRole} = require('./commonHelper');
...
module.exports = {
    ...
    getUserPermissions: async (user)=>{
        ...
        if (user && user.userRoles && user.userRoles.length > 0) {
            ...
            userPermission =  [ ...new Set([].concat.apply([], userRoles?.[0]?.[0]?.permission ?? []))]   ;
        }
                            ...
                            if (modulePermissionData.length > 0 && modulePermissionData !== undefined) {
                                ...
                                _.map(modulePages[moduleID], function (p, key) {
                                    ...
                                    if (submodulePermissionInsideData.length > 0) {
                                        if (_.includes(userPermissions, moduleID + '.' + key + '.' + 'view')) {
